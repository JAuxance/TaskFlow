...
export function renderDirectMessages(member, workspace, navigation) {
    ...
    content.innerHTML = `
        <header class="page-heading page-heading-row">
            ...
            <button type="button" class="btn-secondary direct-back">Back to members</button>
        </header>
        <section class="direct-chat-panel" aria-label="Private conversation"></section>`;
    const name = member.user_name || member.user_email || "Member";
    content.querySelector(".direct-recipient").textContent = `Conversation with ${name}`;
    content.querySelector(".direct-back").addEventListener("click", () => navigation.renderWorkspace(workspace, "members"));
    renderChat(content.querySelector(".direct-chat-panel"), user, navigation, {
        ...
        title: name,
        description: "Only you and this member can access this conversation. A shared workspace is required.",
